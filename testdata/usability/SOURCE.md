...
| `tools/sapient_loopback.py` | The SAPIENT loopback fixture US-09 needs (added 2026-09-08): a generic "always accept" SAPIENT sensor a moderator runs alongside the session, answering whatever `task_id` it receives with `Accepted` on the same connection. Neither `round-1.json` nor `round-1-node.json` has a `sapient_feeds` entry pointed at it yet -- that wiring is a session-setup step, not something the script does |
