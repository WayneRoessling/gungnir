...
| `eframe`, `egui` | 0.34 | Window shell and 2D panels | Runs with `Renderer::Glow`; the optional `wgpu` feature is off. Moved from 0.29 on 2026-09-17 to clear quick-xml 0.30 (RUSTSEC-2026-0194/0195), which only egui-winit 0.29's Linux accessibility chain brought in. eframe is taken without default features, because 0.34's defaults swapped the `glow` renderer for `wgpu`: the list is 0.29's default set less `winit/default`, which leaves Windows unchanged and drops winit's Adwaita title bar on GNOME Wayland. 0.34 is the ceiling while three-d 0.19 sets the shared `glow` |
| `three-d` | 0.19 | 3D viewport | Renders through OpenGL via `glow`, not through `wgpu`: the viewport draws into eframe's context, so three-d's `glow` must be eframe's, and at three-d 0.19 and eframe 0.34 it is one `glow` 0.17, shared with `egui_glow` too (asserted at compile time in `gungnir-app/tests/gl_attachment.rs`). Moved from 0.18 with eframe on 2026-09-17. Taken without its default `window` feature since 2026-09-07: the viewport never opens a three-d window, and the feature carried a second `winit` (0.28) with `glutin` and `instant` (RUSTSEC-2024-0384). Its `cgmath` (RUSTSEC-2026-0196, unmaintained) is not a feature and 0.19 depends on it as 0.18 did, so it stays until three-d drops it or is replaced |
| `wgpu` | 22 | Compute device in `gungnir-render`, used by `gungnir-data-fusion` | Pinned to the line eframe 0.29's optional wgpu feature used, so that enabling the feature never yielded two versions. **That no longer holds** since eframe 0.34 (2026-09-17): its feature is on `wgpu` 29, so enabling it would link a second `wgpu` beside the compute device's 22, or mean moving the compute device to 29 with it. The feature stays off and this pin did not move. `wgpu-hal` 22 carries its own `glow` 0.13, which never meets the shared OpenGL context |
