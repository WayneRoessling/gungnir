...
| CAP-1.3 Sensor modes and tasking | full | partial | Registry, modes, coverage regions, and the outbound control path are built as of 2026-09-05 (GAP-003, GAP-004): a command is recorded, published, and swept for acknowledgement, and a requested mode is kept apart from the confirmed one. No adapter carries a command to a real sensor (GAP-001) and collection requirements are built and tasked from PN-15 but the list is not rebuilt at startup (GAP-005); DN-11 (`../../design/`) names a component for every remaining part | GAP-003, GAP-004, GAP-005, GAP-115 |
| CAP-1.4 Coverage and gaps | full | partial | Coverage volumes in `gungnir-analytics`; gap detection as a function and map rendering of coverage are not designed; a component is now named for every part of it by DN-12 (`../../design/`, first draft 2026-09-05) | GAP-006, GAP-007, GAP-105, GAP-106, GAP-147 |
| CAP-1.5 Time discipline | full | partial | `gungnir-time` clocks and late-data policy; `DetectionView` source and receipt time | GAP-008, GAP-114, GAP-173 |
...
| CAP-2.7 Global identity | full | partial | `gungnir-identity` resolver with lineage; cross-session correlation is an `../../../ARCHITECTURE.md` §10 item | GAP-019, GAP-069, GAP-123, GAP-175 |
...
| CAP-2.11 Geometric questions | full | full | `gungnir-analytics` implemented and tested | GAP-118, GAP-158, GAP-159 |
...
| CAP-3.2 Threat scoring | full | partial | `gungnir-assessment` scoring and reward matrix; class lethality and asset weighting are extensions of the baseline | GAP-027, GAP-028, GAP-124 |
| CAP-3.3 Assignment recommendation | full | partial | `gungnir-allocation` value function and `gungnir-intercept-service`; effector layers and costs have no model in `ResourceConfig`; a component is now named for every part of it by DN-04 (`../../design/`, first draft 2026-09-05) | GAP-029, GAP-030, GAP-097, GAP-119, GAP-156, GAP-157, GAP-168, GAP-169 |
...
| CAP-3.6 Rules of engagement | full | partial | `gungnir-policy` geofence and readiness chain; weapons control status, authority by role and class, escalation and timeout are not designed; a component is now named for every part of it by DN-08 and DN-09 (`../../design/`, first draft 2026-09-05) | GAP-028, GAP-033, GAP-034, GAP-072, GAP-052, GAP-088, GAP-113, GAP-131 |
| CAP-3.7 Queue under saturation | full | partial | `gungnir-command` pending list; ordering by priority and time remaining and pre-delegation are not designed; a component is now named for every part of it by DN-10 (`../../design/`, first draft 2026-09-05) | GAP-034, GAP-035, GAP-132, GAP-140 |
...
| CAP-4.2 Record every decision | full | partial | `InMemoryApprovalWorkflow` records every decision | GAP-028, GAP-130, GAP-131, GAP-132 |
| CAP-4.3 Never execute without a decision | full | full | Recommendation-only rule in `../../gungnir-capabilities.md` §5.4; `PolicyChain` never self-approves | GAP-039, GAP-113, GAP-132, GAP-133 |
...
| CAP-4.6 Track engagements and effects | full | none | `InterceptEvent` in the event schema; no engagement tracking or effect linkage; a component is now named for every part of it by DN-06 (`../../design/`, first draft 2026-09-05) | GAP-043, GAP-135 |
| CAP-4.7 Assist without authority | planned | none | Plan 08 (`../../plans/08-ai-agent-integration.md`) | GAP-044 |
| CAP-5.1 Journal | full | full | `gungnir-store` journal; fsync policy open in `../../performance-budgets.md` | GAP-085, GAP-122, GAP-126 |
| CAP-5.2 Replay and rehearse | partial | partial | `gungnir-replay` over journals; scenario replay through the live pipeline under a chosen plan is not designed; plan 07 for scenarios | GAP-071, GAP-045, GAP-046, GAP-076, GAP-051, GAP-087, GAP-105, GAP-107, GAP-136, GAP-147 |
...
| CAP-5.4 Disconnected and reconcile | full | partial | `gungnir-remote`, `gungnir-resilience`, `gungnir-collab`; mid-session failover waits on the transport | GAP-041, GAP-072, GAP-050, GAP-121, GAP-129, GAP-134, GAP-141, GAP-142, GAP-143, GAP-154, GAP-153, GAP-167 |
| CAP-5.5 Health and alert lifecycle | full | full | `gungnir-observability` and `gungnir-workflow` implemented | GAP-072, GAP-119, GAP-125, GAP-168 |
| CAP-5.6 Baselines and plans | full | partial | `ConfigBaseline` validation and store; asset list, policy configuration, and plan validity periods are not in the schema; a component is now named for every part of it by DN-01 and DN-08 (`../../design/`, first draft 2026-09-05) | GAP-071, GAP-051, GAP-052, GAP-128, GAP-162 |
| CAP-5.7 Model governance | full | partial | `gungnir-modelops` registry, wired 2026-09-05 (GAP-086, DN-24): the baseline declares mission profiles and candidate baselines, both binaries build the registry through the real promotion state machine and journal what the session opened with, and a rollback names the candidate it restored. Applying a promoted configuration to the picture waits on GAP-011 | GAP-077, GAP-078, GAP-079, GAP-081, GAP-083, GAP-086, GAP-053, GAP-067, GAP-137, GAP-138 |
...
| CAP-6.5 Supply chain | full | partial | `deny.toml`, `release.yml`, `../../release-governance.md` | GAP-081, GAP-061, GAP-094, GAP-144 |
...
| CAP-7.1 Versioned interface | full | partial | `../../gungnir-api-v1.md` contract; the read paths are served over HTTP and a WebSocket as of 2026-09-05 (GAP-041), loopback only and with the write paths refusing until a caller can be authenticated | GAP-041, GAP-063, GAP-066, GAP-112, GAP-139, GAP-153, GAP-171 |
| CAP-7.2 Interop standards | full | partial | `SchemaCatalog` with Arrow; codecs pending | GAP-069, GAP-063, GAP-064, GAP-100, GAP-116, GAP-117, GAP-130, GAP-175 |
| CAP-7.3 Three profiles | full | partial | `../../../ARCHITECTURE.md` §8 profiles; both binaries run | GAP-041, GAP-110, GAP-120, GAP-160, GAP-161, GAP-157, GAP-165, GAP-169, GAP-170 |
| CAP-7.4 Peer and coalition exchange | full | none | API and interop provide the mechanism; releasability and peer merge are not designed; a component is now named for every part of it by DN-18 (`../../design/`, first draft 2026-09-05) | GAP-065, GAP-091, GAP-145, GAP-146, GAP-150, GAP-164, GAP-166, GAP-171 |
