...
| CAP-5.6 Baselines and plans | full | partial | `ConfigBaseline` validation and store; asset list, policy configuration, and plan validity periods are not in the schema; a component is now named for every part of it by DN-01 and DN-08 (`../../design/`, first draft 2026-09-05) | GAP-071, GAP-051, GAP-052, GAP-128 |
...
| CAP-6.2 Authorize by role, class, layer | partial | partial | `role_permits` coarse matrix; per-class and per-layer refinements are gaps (`roles-and-stakeholders.md` §4) | GAP-068, GAP-058, GAP-111, GAP-127, GAP-132 |
| CAP-6.3 Audit | full | partial | `gungnir-security` audit log; wiring pending | GAP-059, GAP-111, GAP-152 |
