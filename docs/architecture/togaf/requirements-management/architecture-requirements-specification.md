...
Status: first draft, 2026-09-04. Requirements management. Sixty-four requirements with
...
Thirteen of sixty-four are verified today, and ten of those thirteen are constraints or
...
registry's requirements from these tables (GAP-083). The sentence above counted against
the old total until 2026-09-15.
...
| Capability | Criterion comes from | Which means |
|---|---|---|
| CAP-1.6 Receive early warning, tracks, and reports from peers | Verification table | a row in `../../../verification-capability-table.md` states the criterion |
| CAP-2.3 Register sensors against each other | Increment 2 | the roadmap has it *none* at increment 1, full at increment 2 |
| CAP-2.4 Track dense groups and count targets | GAP-015 | open in the gap register; the criterion arrives with the closure |
...
| CAP-2.10 Fuse terrain, imagery, point clouds, and map layers | Verification table | a row in `../../../verification-capability-table.md` states the criterion |
...
27 of 56 leaf capabilities. The count is a ratchet in `build_uaf.py`: writing a requirement for one
