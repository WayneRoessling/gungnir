...
| `time.late_data: LateDataPolicy` (GAP-114, 2026-09-26) | D-98, D-99 | **Absent means a one-second reorder buffer**, which is what every deployment ran before the field existed, so an upgrade changes nothing the picture does. `buffer-and-reorder` with `max_lateness_s` above zero and at most 10 s, or `reject`; `accept-as-is` is refused, being for replay and testing only. The pipeline and the clock-skew estimate both read this one value |
