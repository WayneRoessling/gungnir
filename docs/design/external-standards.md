...
| `gungnir_ingest::adapters::misb::UasMetadataAdapter` (GAP-099, 2026-09-08) | Buffer a KLV byte stream from a TCP source or a recording, decode complete frames off the front, place a fix (Sensor Latitude/Longitude/[Altitude]) in the local ENU frame and emit it as a `DetectionView` through the real gateway exactly as every other feed's position does, hand every accepted frame's full report to a side-channel sink, and enforce MISB ST 0601.8-08's checksum-discard rule (a mismatched frame produces neither a detection nor a report, counted by name) | Decode, transport, or display the video itself -- out of scope by the design survey's own separation of a video transport from a detection message |
| `gungnir_config::MisbFeedConfig`/`MisbSource`, `gungnir-node::bind_misb_feeds`, `gungnir_app::misb::bind_feeds` (GAP-099, 2026-09-08) | Name a `misb_feeds` entry (a sensor, a `Tcp{addr}`/`File{path}` source), validated the same way `ais_feeds`/`adsb_feeds` already are, and bind every configured feed into the gateway at start in both binaries, on the shared `ConfigBaseline::origin` local frame every other cooperative feed already places into | Attach a `PlatformReportSink` on either binary: nothing yet drains a `UasPlatformReport`, and evidence fusion over one was never named in this gap's own closing action, so an attached, undrained queue would only grow. `gungnir-app` publishes each feed's counters (`MisbStatsSink`) for a future PN-09 row; `gungnir-node` attaches neither sink, matching how its own AIS/ADS-B bindings attach neither |
...
The wiring (2026-09-08) adds one `gungnir-config` test covering `validate_misb_feeds`'s
acceptances and every refusal path, two `gungnir-node` tests, and two
`gungnir-app/tests/misb_feed.rs` tests, all proving a configured `misb_feeds` entry with
