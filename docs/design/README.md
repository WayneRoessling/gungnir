...
**DN-29 picks up the second of those two findings.** `measurement_noise_var` now reaches
...
deployment shared before this note. Proposed, built and signed by the owner the same week
as DN-28: it still touches `gungnir-fusion-async`'s low-trust tier, even though the
change itself is mechanical (a new argument to an existing function, no new estimator, no
new concurrency), and the tier's review was sought rather than assumed unnecessary from
that alone.
