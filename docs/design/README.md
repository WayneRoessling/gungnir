...
| DN-32 Re-observation for a laydown | **Design only; no code exists.** Proposed 2026-09-16, landed 2026-09-17, and walked with the owner 2026-09-17: every §5 recommendation, §5.5's frame and all five §6 containment mechanisms; §10's rows not yet agreed. What is signed is in `../signatures.md` | `DN-32-re-observation-for-a-laydown.md`. GAP-105's design, per D-64: a rehearsal re-observes a fixture's recorded truth with the deployment's own sensor models at the selected laydown's placements, inside the production binary. Adds `gungnir-sensor-sim` and two dependency edges, narrows rather than deletes `scenario_misuse`, adds three fixture sidecars, and contains synthetic detections five independent ways. Four sub-decisions (§5) and the containment design (§6) are what a signature would cover. Drafted as `DN-31` and renumbered: that number, its `D-53` and its edge letters were taken while it sat unmerged (`../record/2026-09-17/gap-105-laydown-draft-and-dn-31-reuse.md`) |
