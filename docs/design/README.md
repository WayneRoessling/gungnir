...
| DN-18 Coalition exchange | **Wired both ways for the picture, and the write path is built** (GAP-065, 2026-09-06; amendment 2 2026-09-08, written and gated where human-owned; amendment 3 2026-09-23) | `gungnir-model/src/exchange.rs`, the two independent gates, composed in `gungnir-api` for a machine caller: `exchange` agreements in the baseline, a party with none refused with the reason. Inbound through the peer link since later the same day (GAP-009). **Amendment 1 (§9, three `/v2/exchange` routes), 2026-09-06**, recording that §6's "no new endpoints" was diverged from: `Warnings`, `Reports` and `Handoffs` had no producer and no gate, and they cannot ride a snapshot. **Amendment 2 (§10)** answers what amendment 1 left open: `POST` on the same three paths (`gungnir_security::actions::PUBLISH_EXCHANGE`, human-owned), store-and-forward mirroring `gungnir-remote`'s sensor-task outbox, and one producer -- the handoff builder republishes the whole handoff set on every new one, through its host (`gungnir-approval/src/handoffs.rs` since GAP-131, `gungnir-app/src/handoffs.rs` before it). `Warnings` and `Reports` still have no producer. **Amendment 3 (§11), 2026-09-23** (GAP-137, D-68): the register is keyed by item and then by producer, so a publish replaces that producer's set alone and a read merges every producer's -- the node's own approval queue is the second producer, and its handoffs reach a partner |
...
| [DN-18 Coalition exchange](DN-18-coalition-exchange.md) | GAP-065, GAP-137 | `gungnir-api` (`ExchangeProducer`, §11) | None |
