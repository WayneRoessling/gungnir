...
| DN-11 Sensor control and tasking | **Implemented and wired** (amendment 1 signed 2026-09-05); **amendment 2 (§10, the first `SensorControlAdapter`, SAPIENT) written 2026-09-07 and unsigned** | `gungnir-sensor-management/src/lib.rs` (`SensorControl`) and `tasking.rs`, `sapient_task.rs`, `gungnir-model/src/requirements.rs`, `gungnir-workflow/src/tasking_case.rs`, PN-10 and PN-15 in `gungnir-ui`, `gungnir-app/src/requirements.rs`. A command travels to the node and is closed by the acknowledgement it streams back (GAP-004, 2026-09-06); the wire from the node to a sensor waits on GAP-001 |
