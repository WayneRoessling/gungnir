...
| §5 rule 1 | `gungnir_fusion_async::{BearingDetection, FusionPipeline::offer_bearing}`, with `gungnir_filters::{BearingOnly, AzimuthElevation}` for the update. The prohibition is a **type boundary**: no function takes a bearing and creates a track | A bearing is applied at the pipeline's current cursor, not retrodicted; one outside the bearing window (the reorder horizon, at least one second) is refused and counted, and under a `Reject` late-data policy one behind the cursor is refused as late (GAP-114) |
