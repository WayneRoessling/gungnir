...
| Known vulnerabilities | `cargo deny check advisories` and `cargo audit` on every release, both against the one list of accepted advisories in `deny.toml` (`.github/scripts/deny_advisory_ignores.py`); `fuzz-nightly.yml` for our own parsers | `release.yml`, `fuzz-nightly.yml` |
