...
| Known vulnerabilities | `cargo deny check advisories` on every pull request (`ci.yml`) and every release; `cargo audit` on every release; both against the one list of accepted advisories in `deny.toml` (`.github/scripts/deny_advisory_ignores.py`); `fuzz-nightly.yml` for our own parsers | `ci.yml`, `release.yml`, `fuzz-nightly.yml` |
