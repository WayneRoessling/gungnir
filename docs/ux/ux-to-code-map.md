...
|---|---|---|---|---|
| PN-01 Status strip | `gungnir-ui/src/panels/status_strip.rs` (new) | `backend`, `mission`, `clock`, `health`, alert counts, role; outbox counts and `last_heard_age` from the remote service (D-23); the operator line from the session authority: who, their role, the time left, or why nobody (GAP-057, 2026-09-06); whose approval queue is in force, the node's or this desktop's, from `projection::authority` (GAP-133, DN-31 §8) | nothing | GAP-072 |
| PN-02 Viewport | `gungnir-viewport3d/src/lib.rs::render`, `tracks.rs` | `tracking.tracks()`, `tracking.bearing_rays()`, `last_plan`, `viewport` | selection, camera | layers GAP-007; hazards GAP-017 (`layers::draw_hazards_2d`, from `gungnir-app/src/hazards.rs`); geofences GAP-088 (`layers::draw_geofences_2d`, from `gungnir-app/src/geofences.rs`); scene GAP-022; classification frames GAP-073; bearing rays built 2026-09-08 (GAP-096, DN-27 §7: `tracks::draw_bearing_rays_2d` draws each as a wedge from the sensor along the azimuth, widening with the angular one-sigma and reaching past the visible rectangle rather than terminating; drawn in the 2D projection only, since the three-d GL scene draws no glyphs for anything yet, tracks included) |
...
| PN-05 Recommendation | `gungnir-ui/src/panels/intercept_panel.rs` (extend) | `last_plan`; `withheld` from `InterceptService::withheld` (GAP-030); `fires_checks` from `decisions::fires_checks` (GAP-036); `handoffs` from `AppState.handoffs` (GAP-040); verdict and rationale (`CourseOfAction`); alternatives | opens PN-07 | built for the plan, the withheld list and the fires checks; rationale and alternatives are GAP-032. A plan and a decision are named by their short tag, `…9f3a61c2` (D-61, GAP-130) |
| PN-06 Approval queue | `gungnir-ui/src/panels/approval_queue.rs` | **whose queue is in force decides where the rows come from** (GAP-133, DN-31 §6.6): while linked, the node's queue from `projection::queue_rows` over `gungnir_remote::queue`, in the node's order, with what has ended on it beside them; while cut off, pending approvals from `InMemoryApprovalWorkflow`, which the tick feeds | selection | built; the queue is always empty in this build and says why. Time remaining, expiry, escalation marks and ordering are real (GAP-034, GAP-035); the priority tie-break needs GAP-028 and the panel says which ordering it is showing. Rows and outstanding handoffs name a plan or a decision by its short tag, `…9f3a61c2`, and a rehearsal seed's plan by its seed number (D-61, GAP-130). Since GAP-133 the panel says whose queue it is drawing, a row this role may not decide names the roles it is offered to, a queue the node has not yet sent says so rather than reading as empty (`EmptyBecause::NotReceived`), what has ended on the node is drawn with who ended it, and with nobody signed in a linked console is read-only because the node's route needs a token |
| PN-07 Decision dialog | `gungnir-ui/src/panels/decision_dialog.rs` | the selected plan and verdict; degraded conditions from the health flags; the policy chain that produced the verdict; where the decision goes (`DecisionRoute`) and what the node answered (`NodeAnswer`), GAP-133 | `POST /v3/queue/{item}/decision` through `projection::decide` while linked, `ApprovalWorkflow::decide` while cut off | built; accept is gated on acknowledging the degraded conditions and is drawn last; rationale, alternatives and cost need GAP-028 and GAP-032; the record names no operator until GAP-057. The plan under decision is shown whole, with a copy control, beside its short tag (D-61, GAP-130). Since GAP-133 a linked dialog says the node takes the decision and records nothing here; a `409` shows who decided, as which role and when, and draws no control; a `400`, `401` or `403` says nothing was recorded on either side; a post still in flight says so, and says how many times it has been sent under the same request key |
| PN-08 Alerts and incidents | `gungnir-ui/src/panels/alerts.rs` (replace the string list) | `AlertLifecycle` list from `gungnir-workflow`; correlation from `gungnir-observability` | `AlertLifecycle::transition` | GAP-055; a retained bearing that classifies but does not localise belongs here rather than on the map (DN-27 §7), built 2026-09-08 as one alert per newly retained bearing (`gungnir-app/src/bearings.rs::tick`, naming the sensor and azimuth; the string list itself is still GAP-055's to replace) (GAP-096) |
| PN-09 System health | `gungnir-ui/src/panels/sensor_health.rs` (extend) | `health`; `SensorRecord`s; `clock_skew.health` per source (GAP-008); `anomaly::detector_status` (GAP-021); the terrain line (GAP-023); each radar feed's counters from its `FeedStatsSink` (GAP-001, 2026-09-06); each AIS feed's counters and match tally (GAP-010); each peer link's state (GAP-009); each SAPIENT feed's counters from its `SapientFeedStatsSink`, and `tracking.pipeline_stats()`'s five bearing counters (GAP-096, 2026-09-08); journal and outbox state | nothing | built for health, skew, detectors, terrain, radar and AIS feeds, peer links, and now the spotter/acoustic/passive-RF feed line with the pipeline's bearing counters beside it (`BearingFeedLine`, `BearingPipelineLine`, GAP-096), through one `SensorHealthView`; journal state not drawn, the outbox is on PN-18 |
...
| PN-17 Commander summary | `gungnir-ui/src/panels/commander_summary.rs` | queue statistics -- the node's while linked, from `projection::node_queue_stats`, and this desktop's while cut off, with the panel told which (GAP-133, DN-31 §8) -- delegations, accepted gaps, plan in force, `engagements::outcome_counts` (GAP-043), `sustainment::exposure_lines` (GAP-026) | delegate, accept gap | built; plan in force and delegations are real, queue GAP-038, accepted gaps GAP-006, outcomes are real (GAP-043); no controls drawn until GAP-035. Since GAP-133 the queue line carries expiries, escalations and decisions by role for the period, and says whose queue they are about: a commander asking who is deciding what is asking about the watch, not about one console |
| PN-18 Reconciliation | `gungnir-app/src/workspace.rs` (`render_reconciliation`, `render_both_acted`, `render_delegations`, `render_forwarding`) over `gungnir-app/src/failover.rs` | **both-acted incidents first, above everything else** (D-58): each track engaged on both sides of the outage, with both engagements' plans and times and no button; then the outage's bounds and the decisions taken meanwhile; where D-15's delegations stand (in force until, or lapsed at); the `gungnir-resilience` merge over the desktop's journal and the node's `GET /v3/history`: envelopes per side, duplicates dropped, each conflicting decision by plan and who settled it; or why the node's half is unavailable; and where the outage's forwarding stands -- waiting on a conflict, sent, held by the node with its counts, or refused | `PanelAction::SwitchBack` once every conflict is resolved, the node has not refused the outage, and the node answers (D-15); `PanelAction::ResolveConflict` per conflict, through the authority check onto the record | built 2026-09-06 (GAP-050): fallback, report, per-conflict resolution, the store-and-forward line and switch back; **2026-09-17 (GAP-134)**: both-acted incidents, the delegation lapse and the forwarding line |
| PN-19 Assistant | `gungnir-ui/src/panels/assistant.rs` (new, plan 08) | `gungnir-agent` conversation | question text | GAP-044 |
| PN-20 Audit and accounts | `gungnir-ui/src/panels/audit.rs` | `session_state()`, `AppState.accounts`, `sustainment::audit_lines` (`session::audit_view`) | `session::apply` for sign in, sign out and `AssignRole` (an administrator's act, on the audit trail) | built (GAP-057, GAP-059); the marking row says no marking changes can be made (GAP-062); each handoff record shows its decision and plan whole, with copy controls (D-61, GAP-130) |
