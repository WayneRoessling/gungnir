...
//! A lossless JSON form for an envelope that carries a non-finite float: the journal's
//! line (GAP-126, decision D-77) and, since GAP-153, the v3 wire's frame and body
//! (decision D-96, `docs/gungnir-api-v1.md`'s "Non-finite floats on the wire").
//!
//! # Why this exists
//!
...
//!
//! **The wire had the same defect one hop later** (GAP-153): the node sent each envelope
//! to its desktops, and served `GET /v3/history` and `GET /v3/snapshot`, as plain
//! `serde_json`, so the value the journal had just kept went out as `null`, the desktop
//! could not decode the frame and took it for the node ending the stream. Both ends of the
//! link now use this form, which is why it lives here, beside [`crate::Envelope`], and not
//! in `gungnir-store`: the journal, the node's transport and the desktop's link each
//! depend on this crate, and none of the other two may depend on the store.
//! `gungnir_store::nonfinite` re-exports it, so the journal's own callers are unchanged.
//!
//! # The encoding
//!
...
/// line starts with `{` and a sealed one with `#` (`gungnir-store/src/sealing.rs`), so the
/// three never meet; on the wire a marked frame or body starts with it where a plain one
/// starts with `{`.
...
/// Encode a value and prove the line reads back as the value that wrote it: decoded and
/// encoded again, a marked line must be the same line, bit for bit in every float.
///
/// The journal refuses an envelope that fails this (`StoreError::Unfaithful`, D-77), and
/// since GAP-153 the node's transport refuses to offer one (D-96): a line that would not
/// come back as written is a frame a desktop would misread or could not read at all. A
/// plain line is `serde_json`'s own output, which reads back by that crate's contract
/// (with `float_roundtrip`, §2.9), so only a marked line pays for the second pass.
///
/// # Errors
///
/// Why the value has no faithful line: `serde_json` refused it, or the marked line decodes
/// to something that encodes differently.
pub fn to_faithful_line<T>(value: &T) -> Result<String, String>
where
    T: Serialize + de::DeserializeOwned,
{
    let line = to_line(value).map_err(|e| format!("it cannot be encoded: {e}"))?;
    if line.starts_with(MARKER) {
        let back: T = from_line(&line).map_err(|e| format!("the line does not decode: {e}"))?;
        let again = to_line(&back).map_err(|e| format!("the decoded value re-encodes: {e}"))?;
        if again != line {
            return Err("the line decodes to a different value".into());
        }
    }
    Ok(line)
}
...
mod tests {
    ...
    /// A value whose `Deserialize` does not undo its `Serialize`: it reads a NaN back as
    /// zero. No type on the wire is known to do this; it is what the read-back proof exists
    /// to catch, so the proof is tested against one.
    #[derive(Debug)]
    struct Lossy(f64);
    ...
    impl<'de> serde::Deserialize<'de> for Lossy {
        fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
            ...
            Ok(Lossy(if v.is_nan() { 0.0 } else { v }))
        }
    }

    /// GAP-153, D-96: what the node's transport offers is a line proved to read back. A
    /// finite value is `serde_json`'s own line and a non-finite one the marked line; a
    /// marked line that decodes to something else, or a value `serde_json` cannot write,
    /// is refused with the reason rather than sent.
    #[test]
    fn a_faithful_line_is_proved_to_read_back_and_an_unfaithful_one_is_refused() {
        let finite = Plain {
            x: 1.0,
            y: None,
            s: String::new(),
        ...
        assert_eq!(
            to_faithful_line(&finite).expect("faithful"),
            serde_json::to_string(&finite).expect("serde_json")
        );
        let diverged = Plain {
            x: f64::NAN,
            y: Some(f64::INFINITY),
        ...
        assert!(to_faithful_line(&diverged)
            .expect("faithful")
            .starts_with(MARKER));

        // A finite `Lossy` is plain JSON, which `serde_json` reads back by its own
        // contract; a NaN one is marked, and reads back as a different value.
        assert!(to_faithful_line(&Lossy(2.0)).is_ok());
        let why = to_faithful_line(&Lossy(f64::NAN)).expect_err("refused");
        assert!(why.contains("different value"), "{why}");

        // A map `serde_json` cannot key.
