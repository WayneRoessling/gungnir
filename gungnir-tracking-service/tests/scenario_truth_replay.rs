...
//!   measurement noise (DN-29); the figure this row carried before DN-29 (238 m) was run
...
//!   cross-range noise (DN-29); its height axis is left at the default's placeholder --
...
//!   single-sensor scenario's own measurement noise (DN-29) rather than the generic
//!   figure these counts were first recorded against**: scenario 1 ends with two tracks
//!   for one aircraft, none confirmed (three before DN-29, over the same scenario and
//!   filter -- most of the fragmentation this row first blamed on the motion model was
//!   the noise mismatch DN-28 §7 found, not a defect DN-29 closes); scenario 2 is
...
/// cannot carry that rotation. DN-29 follows DN-28's own precedent rather than
...
/// set its scenario's own measurement noise (DN-29) rather than leaving every one on
...
///
/// Runs with `radar_medium`'s own measurement noise (DN-29), not
...
///
/// Runs with `radar_coastal`'s own range and cross-range noise (DN-29). Its height axis
...
///
/// Each single-sensor scenario runs with its own sensor's measurement noise (DN-29); the
